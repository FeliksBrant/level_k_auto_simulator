...
// compute acceleration component parallel to the track.
float SingleCardata::getAccel(tCarElt *car, float ltrackangle)
{
	vec2f accel, dir;
	accel.x = car->pub.DynGCg.acc.x;
	accel.y = car->pub.DynGCg.acc.y;
	dir.x = cos(ltrackangle);
	dir.y = sin(ltrackangle);
	return accel*dir;
}

// compute pos component parallel to the track.
float SingleCardata::getPosition(tCarElt *car, float ltrackangle)
{
	vec2f pos, dir;
	pos.x = car->_pos_X;
	pos.y = car->_pos_Y;
