...
class Driver : public L1_Policy {
	...
	private:
		...
		float getBrake();
		int getGear();
		float getSteer(int targetLaneType);
		float getClutch();
		vec2f getTargetPoint(int targetLaneType);
		float getOffset();
		...
		// void LKController(int policy_type);
		void LKconstraints();
	...
	//my definitions
		...
		static const double lateral_speed; // set later speed for lane change m/s
