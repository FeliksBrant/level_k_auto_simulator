...
recordMisc record_Misc = {"Player data/", "SimTime\tAction\tObservation\tdist_lon\tdist_lat\tvel_lon\tvel_lat\taccel_lon\taccel_lat\tyaw\tyaw_dot\n", "0", 0, -1, 0, 0, 0, 0, 0};
DASmisc DAS_misc = {0};
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	record_init(record_Misc, myfile, curTrack);
...
static void common_drive(int index, tCarElt* car, tSituation *s)
{
	...
	record_main(car, s, curTrack, myfile, record_Misc);
	DAS_main(car, HCtx[idx], DAS_misc);
}
