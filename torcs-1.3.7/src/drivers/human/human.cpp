...
DASmisc DAS_misc = {0,0,0,0,0,0,0,0,0,false};
...
void newrace(int index, tCarElt* car, tSituation *s)
{
	...
	if(s->LKinfo.wantRecord){
		recordMisc_init(&(HCtx[idx]->record_Misc),"Experiment data/", "SimTime\tAction\tObservation\tdist_lon\tdist_lat\tvel_lon\tvel_lat\taccel_lon\taccel_lat\tyaw\tyaw_dot\tfl_d\tfl_v\tfc_d\tfc_v\tfr_d\tfr_v\trl_d\trl_v\trr_d\trr_v\tlane\tcollision\n", "0", 0, -1, 0, 0, 0, 0, 0);
		record_init(HCtx[idx]->record_Misc, myfile, curTrack, s, -1, 0);		
	}

	DAS_misc.lanePos = classify_lanepos(car);
...
static void common_drive(int index, tCarElt* car, tSituation *s)
{
	...
	if(s->LKinfo.wantRecord){
		record_main(car, s, curTrack, myfile, HCtx[idx]->record_Misc, &obsData);
	}
	DAS_main(s, car, HCtx[idx], DAS_misc, obsData);
}
